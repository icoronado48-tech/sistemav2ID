...
class UpdateMateriaPrimaRequest extends FormRequest
{
     ...
     * Autoriza la actualización de una materia prima existente usando la MateriaPrimaPolicy.
     *
     * @return bool
     */
    public function authorize(): bool
    {
        ...
        // para el método 'update', pasando la instancia del modelo MateriaPrima de la ruta.
        return Auth::user()->can('update', $this->route('materia_prima'));
    }
    ...
    {
        $materiaPrimaId = $this->route('materia_prima')->id;

        return [
            // El nombre debe ser único, pero ignorando la materia prima actual.
            'nombre' => ['required', 'string', 'max:255', Rule::unique('materias_primas', 'nombre')->ignore($materiaPrimaId)],
