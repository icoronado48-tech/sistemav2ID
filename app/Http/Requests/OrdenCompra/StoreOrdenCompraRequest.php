...
class StoreOrdenCompraRequest extends FormRequest
{
     ...
     * Autoriza la creación de una nueva orden de compra usando la OrdenCompraPolicy.
     *
     * @return bool
     */
    public function authorize(): bool
    {
        ...
        // para el método 'create', pasando la clase del modelo OrdenCompra.
        return Auth::user()->can('create', OrdenCompra::class);
    }
    ...
    public function rules(): array
    {
        return [
            ...
            // 'creada_por_user_id' => ['required', 'integer', 'exists:users,id'], // Será asignado por el controlador (Auth::id())
            'fecha_orden' => ['required', 'date'],
            'fecha_entrega_estimada' => ['nullable', 'date', 'after_or_equal:fecha_orden'],
            'estado' => ['required', 'string', 'in:Pendiente,Aprobada,Rechazada,Completada'], // Estados permitidos
            'observaciones' => ['nullable', 'string', 'max:1000'],
            // Reglas para los detalles de la orden de compra
            'detalles' => ['required', 'array', 'min:1'], // La orden debe tener al menos un detalle
            'detalles.*.materia_prima_id' => ['required', 'integer', 'exists:materias_primas,id'], // ID de materia prima existe
            'detalles.*.cantidad' => ['required', 'numeric', 'min:0.01'], // Cantidad positiva
    ...
    public function messages(): array
    {
        return [
            ...
            'detalles.required' => 'La orden de compra debe contener al menos un producto.',
            'detalles.array' => 'Los detalles de la orden deben ser un array.',
            'detalles.min' => 'La orden de compra debe contener al menos :min producto.',
