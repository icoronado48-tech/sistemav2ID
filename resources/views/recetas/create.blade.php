        ...
        <div class="container-fluid">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Formulario de Receta</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ route('recetas.store') }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('producto_terminado_id') is-invalid @enderror"
                                id="producto_terminado_id" name="producto_terminado_id" required>
                                <option value="">Seleccione un producto terminado</option>
                                @foreach ($productosTerminados as $producto)
                                    <option value="{{ $producto->id }}"
                                        {{ old('producto_terminado_id') == $producto->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('nombre_receta') is-invalid @enderror"
                                id="nombre_receta" name="nombre_receta" value="{{ old('nombre_receta') }}"
                                placeholder="Ej: Receta de Pastel de Chocolate" required>
                            @error('nombre_receta')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" name="descripcion"
                                rows="3" placeholder="Descripción detallada de la receta">{{ old('descripcion') }}</textarea>
                            @error('descripcion')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div id="ingredientes_container">
                            {{-- Los ingredientes se añadirán aquí dinámicamente --}}
                            @if (old('ingredientes'))
                                @foreach (old('ingredientes') as $index => $ingrediente)
                                    @include('recetas.partials.ingrediente_row', [
                                        'index' => $index,
                                        'materiasPrimas' => $materiasPrimas,
                                        'selectedMateriaPrimaId' => $ingrediente['materia_prima_id'],
                                        'cantidadNecesaria' => $ingrediente['cantidad_necesaria'],
                                    ])
                                @endforeach
                            @else
                                {{-- Fila inicial si no hay old input --}}
                                @include('recetas.partials.ingrediente_row', [
                                    'index' => 0,
                                    'materiasPrimas' => $materiasPrimas,
                        ...
                        <button type="button" class="btn btn-info btn-sm mt-3" id="add_ingrediente_btn"><i
                                class="fas fa-plus"></i> Añadir Ingrediente</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            let ingredienteIndex =
                {{ old('ingredientes') ? count(old('ingredientes')) : 1 }}; // Inicia el índice para nuevas filas
            ...
            console.log('Ingrediente Index inicial:', ingredienteIndex);

            // Función para añadir una nueva fila de ingrediente
            document.getElementById('add_ingrediente_btn').addEventListener('click', function() {
            ...
            document.getElementById('ingredientes_container').addEventListener('click', function(event) {
                if (event.target.classList.contains('remove-ingrediente-btn') || event.target.closest(
